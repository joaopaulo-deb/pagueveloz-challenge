...
{
    public class CaptureOperationTests
    {
        ...
        private CaptureOperation CreateSut()
        ...
        private static Account CreateAccountWithReserve(int id = 1, int availableBalance = 100, int reserved = 40)
        {
            ...
            acc.Reserve(reserved);
            return acc;
        }

        private static TransactionInputDto CreateDto(int amount = 10, string reference = " ref-123 ")
        ...
        {
            var account = CreateAccountWithReserve(id: 1);
        ...
        [Fact]
        public async Task ExecuteAsync_HappyPath_CapturesReserved_Commits_Publishes_UpdatesTxSuccess()
        {
            var account = CreateAccountWithReserve(id: 1, availableBalance: 100, reserved: 40);
            var dto = CreateDto(amount: 15, reference: " ref-123 ");
            ...
            Assert.Equal(25, result.reserved_balance);

            Assert.NotNull(pending);
            Assert.Equal("Captura da reserva", pending!.Description);
        }
